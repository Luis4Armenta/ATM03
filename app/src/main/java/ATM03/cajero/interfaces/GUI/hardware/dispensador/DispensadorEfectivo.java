...
public class DispensadorEfectivo {
  ...
  public DispensadorEfectivo(Billetera... depositos) {  // Recibimos billeteras de cada denominación
    this.billeteras = depositos;
  }

  public boolean dispensar(int cantidad) {
    int necesito = cantidad;  // Se define la cantidad que se necesita recolectar en billetes de todo tipo para cumplir

    if (cantidad % 100 == 0 && cantidad > 0) {  // verificamos si la cantidad solicitada es multiplo de 100
      for (Billetera billetera : this.billeteras) { // recorremos todas nuestros contenedores de billetes

        // Si un contenedor de billetes está vacío simplemente lo saltamos
        ...
        if (necesito % billetera.obtenerDenominacion() >= 0 && cantidad >= billetera.obtenerDenominacion()) {
          ...
          // indicamos la cantidad de dinero que todavía necesitamos
          necesito = necesito - (billetesRecolctados * denominacion);
        }
      }

      // comprobamos si hemos logrado juntar billetes para dispensar la cantidad indicada
